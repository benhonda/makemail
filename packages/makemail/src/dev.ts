...
export default async function (config: CLIConfig) {
  ...
  if (config.browserSync) {
    ...
    await $`./node_modules/.bin/browser-sync ${config.dirs.output} -w --startPath ${parseLocaleFileName(
      config,
      config._defaultIndexFile,
      "en",
    )} ${config.browserSync.open ? "" : "--no-open"}`;
  }
}

async function watchOrRead(config: CLIConfig) {
