...
async function getSettingsFile(
  ...
  if (settingsFiles.length === 0) return undefined;
  if (settingsFiles.length > 1) {
    console.log(
      chalk.red(
        `Multiple settings files found: ${settingsFiles.join(
          ", ",
        )}. Please specify one using --settings or set your workspace with makemail dev <workspace>.`,
      ),
    );
...
export async function compileSettings(
  ...
  const definedSettings: () => Promise<UserSettings> = async () => {
    ...
    settings = await getSettingsFile(
      defaultSettings,
      env === "prod" ? `${workspace}/makemail.prod.{json,yml,yaml}` : `${workspace}/makemail.dev.{json,yml,yaml}`,
    );
  ...
  const settings: CompiledSettings = {
    ...userSettings,
    ...
    options: {
      ...
      omitDefaultLocaleFromFileName: parseEnvBoolWithArgv(
        userSettings.options?.omitDefaultLocaleFromFileName,
        opts.omitDefaultLocaleFromFileName,
        env,
      ),
    },
  ...
  if (settings.uploadFilesAndAssets || settings.forceUploadFilesAndAssets) {
    ...
    if (!bucket) {
      console.log(
        chalk.red("No S3 bucket found. Either set s3.bucket in settings or AWS_DEFAULT_BUCKET in your environment."),
      );
      return process.exit(1);
    }
    ...
    if (!region) {
      console.log(
        chalk.red("No S3 region found. Either set s3.region in settings or AWS_DEFAULT_REGION in your environment."),
      );
      return process.exit(1);
    }
...
export async function compileRuntimeConfig(settings: CompiledSettings, env: "dev" | "prod") {
  // get all files
  const runtime = {
    files: {},
    env,
      ...
      for (const [i, locale] of locales.entries()) {
        ...
        const outputPath =
          i === 0 && settings.options?.omitDefaultLocaleFromFileName
            ...
            : `${path.dirname(file.outputPath)}/${path.basename(
                file.outputPath,
                path.extname(file.outputPath),
              )}_${locale}.${outputType}`;
...
export async function recompileFrontMatterForFile(runtime: RunTimeConfig, file: RunTimeFile) {
  // load the file
