...
async function compile(inputFile, config) {
    ...
    for (const locale of locales) {
        ...
        const hostedUrl = config.s3
            ? `https://${config.s3.bucket}.s3.${config.s3.region}.amazonaws.com/${parseLocaleFileName(config, inputFile, locale)}`
            : "#";
        ...
        const outputFile = `${inputFile.replace(config.dirs.templates, config.dirs.output)}/${parseLocaleFileName(config, inputFile, locale)}`;
...
export default async function (config) {
    if (config.preview) {
        ...
        if (config.browserSync) {
            ...
            const outputFile = `${config.preview.replace(config.dirs.templates, "")}/${parseLocaleFileName(config, config.preview, config.locales[0])}`;
            await $ `./node_modules/.bin/browser-sync ${config.dirs.output} -w --startPath ${outputFile} ${config.browserSync.open ? "" : "--no-open"}`;
        }
    }
}
