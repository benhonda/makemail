            ...
            for (const [i, locale] of locales.entries()) {
                ...
                if (env === "prod") {
                    if (settings.s3) {
                        ...
                        handlebars.context[settings.options.viewInBrowserTag] = `https://${settings.s3.bucket}.s3.${settings.s3.region}.amazonaws.com/${settings.s3.path}${path.basename(outputPath)}`;
                    }
